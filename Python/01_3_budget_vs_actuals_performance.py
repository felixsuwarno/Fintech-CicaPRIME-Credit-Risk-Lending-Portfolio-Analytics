...
def _plot_by_year_segments(
    ...
    This breaks the line between Dec -> Jan while keeping the series readable.
    """
...
def plot_actual_vs_budget_with_variance_pct(
    ...
    # Fixed color mapping so each series stays the same color across year segments
    COLOR_ACTUAL = "C0"
    COLOR_BASE = "C1"
    COLOR_STRETCH = "C2"
    COLOR_VAR_BASE = "C0"
    ...
    # -------------------------
    _plot_by_year_segments(ax_top, df, actual_col, "Actual", color=COLOR_ACTUAL)
    _plot_by_year_segments(ax_top, df, base_budget_col, "Budget (Base)", color=COLOR_BASE)
    _plot_by_year_segments(ax_top, df, stretch_budget_col, "Budget (Stretch)", color=COLOR_STRETCH)
    ...
    # -------------------------
    df_pct = df.copy()
    df_pct[var_base_pct_col] = df_pct[var_base_pct_col] * 100
    df_pct[var_stretch_pct_col] = df_pct[var_stretch_pct_col] * 100

    ax_bottom.axhline(0, linewidth=1)

    _plot_by_year_segments(ax_bottom, df_pct, var_base_pct_col, "Variance % vs Base", color=COLOR_VAR_BASE)

    if show_stretch_variance:
        _plot_by_year_segments(ax_bottom, df_pct, var_stretch_pct_col, "Variance % vs Stretch", color=COLOR_VAR_STRETCH)
