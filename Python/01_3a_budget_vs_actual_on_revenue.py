...
actual_revenue_path     = os.path.normpath(os.path.join(data_generated_dir, "01_3a_actual_revenue.csv"))
budget_plan_path        = os.path.normpath(os.path.join(data_raw_dir, "budget_plan_monthly.csv"))

df_actual               = pd.read_csv(actual_revenue_path)
df_budget_raw           = pd.read_csv(budget_plan_path)

# -----------------------------------------------------------
# Prepare actual revenue (monthly)
# -----------------------------------------------------------
...
df_actual["actual_revenue"] = df_actual["actual_revenue"].fillna(0)

# -----------------------------------------------------------
# Prepare budget revenue (monthly) - NO pivot_table
# -----------------------------------------------------------
...
df_budget_base      = df_budget_base.groupby("year_month", as_index=False).agg(planned_revenue=("planned_revenue", "sum"))
df_budget_stretch   = df_budget_stretch.groupby("year_month", as_index=False).agg(planned_revenue=("planned_revenue", "sum"))

df_budget_base      = df_budget_base.rename(columns={"planned_revenue": "budget_base_for_revenue"})
df_budget_stretch   = df_budget_stretch.rename(columns={"planned_revenue": "budget_stretch_for_revenue"})
...
# -----------------------------------------------------------
# Chart (Revenue) - matches your existing output layout
# -----------------------------------------------------------
...
_plot_by_year_segments(ax_top, df_bva, "actual_revenue", "Actual", color=COLOR_ACTUAL)
_plot_by_year_segments(ax_top, df_bva, "budget_base_for_revenue", "Budget (Base)", color=COLOR_BASE)
_plot_by_year_segments(ax_top, df_bva, "budget_stretch_for_revenue", "Budget (Stretch)", color=COLOR_STRETCH)

ax_top.set_title("CICA Prime — Actual vs Budget (Revenue)")
...
# Bottom: Variance (%)
df_pct = df_bva.copy()
df_pct["var_revenue_base_pct"] = df_pct["var_revenue_base_pct"] * 100
df_pct["var_revenue_stretch_pct"] = df_pct["var_revenue_stretch_pct"] * 100

ax_bottom.axhline(0, linewidth=1)

_plot_by_year_segments(ax_bottom, df_pct, "var_revenue_base_pct", "Variance % vs Base", color=COLOR_VAR_BASE)
_plot_by_year_segments(ax_bottom, df_pct, "var_revenue_stretch_pct", "Variance % vs Stretch", color=COLOR_VAR_STRETCH)

ax_bottom.set_title("CICA Prime — Monthly Variance % (Revenue)")
